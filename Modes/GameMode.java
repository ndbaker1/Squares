...
public abstract class GameMode extends Screen{
	...
	public GameMode(){
		...
		player = new Player(Game.SCREEN_WIDTH/2-Player.PLAYER_DIMENSIONS/2,Game.SCREEN_HEIGHT/2-Player.PLAYER_DIMENSIONS/2);
		horde = new ArrayList<Enemy>();
		isAlive = true;

		addMouseListener(new MouseAdapter(){
			public void mousePressed(MouseEvent e)					{	if(e.getButton() == 1)	Game.key_Map.put("SPACE", 	true);	}
			public void mouseReleased(MouseEvent e)					{	if(e.getButton() == 1)	Game.key_Map.put("SPACE",	false);	}
		});
		addKeyListener(new KeyAdapter(){
			public void keyPressed(KeyEvent e){
				...
				if(e.getKeyCode() == KeyEvent.VK_SPACE)										{	Game.key_Map.put("SPACE",	true);	}
				if(e.getKeyCode() == KeyEvent.VK_SHIFT)										{	Game.key_Map.put("SHIFT",	true);	}
			}
			public void keyReleased(KeyEvent e){
				...
				if(e.getKeyCode() == KeyEvent.VK_SPACE)										{	Game.key_Map.put("SPACE",	false);	}
				if(e.getKeyCode() == KeyEvent.VK_SHIFT)										{	Game.key_Map.put("SHIFT",	false);	}
			}
		});
	...
	/**
	 * Moves the Horde of Enemies and Removes them if they are far enough off the screen
	 */
	protected void hordeUpdate(){
		for(int i = horde.size()-1; isAlive && i >= 0 && horde.size() > i; i--)
		{
			...
			if(!e.stunned() && e.collision(player)){
				if(player.isDashing()){
	...
	public void drawComponents( Graphics g ){
		...
		for(Enemy e: horde){
			if(Game.SMOOTH_LIGHTING) e.drawLight(g);
			e.drawBody(g);
		}
		player.draw(g);
