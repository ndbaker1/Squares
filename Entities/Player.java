...
public class Player {
	...
	private static final int DASH_RADIUS = 350;

	private int size, x, y, speed, dash_Tick, stunDuration;
	public double[] dash_vec;
	...
	private void updateMovement() {
		if (dash_vec[2]++ < 5) {
			...
			// adding a 5th of the vector each frame
			x += (dash_vec[0] / 5);
			y += (dash_vec[1] / 5);
		}
	...
	private double[] dashParams() {
		...
		return new double[] { dx, dy, 0, getCenterX(), getCenterY() };
	}
	...
	private void drawTrail(Graphics g) {
		synchronized (trail) {
			for (double[] s : trail) {
				g.setColor(new Color((int) (128 * (Math.cos(s[0] / 20.0) + 1) % 255),
						(int) (128 * (Math.sin(s[0] / 20.0) + 1) % 255), (int) (128 * (Math.sin(s[1] / 20.0) + 1) % 255), 16));
	...
	public void moveWASD() {
		...
		if (Game.key_Map.get("UP") && getCenterY() > 0) {
			VDIR++;
		}
		if (Game.key_Map.get("DOWN") && getCenterY() < Game.SCREEN_HEIGHT) {
			VDIR--;
		}
		if (Game.key_Map.get("RIGHT") && getCenterX() < Game.SCREEN_WIDTH) {
			HDIR++;
		}
		if (Game.key_Map.get("LEFT") && getCenterX() > 0) {
			HDIR--;
		}
		if (HDIR != 0 || VDIR != 0) {
	...
	/**
	 * Resets the Cooldown of the Dash back to Zero and modifies how the cooldown is
	 * drawn
	 */
